...
export class LoginService {
    ...
    login(username, password, autoLogin): Observable<User> {
        let _user: User = null;
        const _filterUser = USERS.filter(function(u) {
            if (u.username === username && u.password === Md5.hashStr(password)) {
                return u;
            }
        });
    ...
    checkAutoLogin(): Observable<User> {
        ...
        if (autoLogin) {
            ...
            let _user: User = null;
            const _filterUser = USERS.filter(function(u) {
                if (u.username === autoLogin['username'] && u.password === autoLogin['password']) {
                    return u;
                }
            });
            if (_filterUser.length > 0) {
